...
const DB = {
    ...
    get(key) {
        if (key === 'submissions') {
            ...
            this.data.submissions.forEach(sub => {
                if (String(sub.taskId) === 'mandatory_cert_task' && sub.status === 'pending' && sub.timestamp) {
    ...
    async registerIntern(data) {
        ...
        const ongoingInternship = this.data.interns.find(i =>
            (String(i.email).trim().toLowerCase() === inputEmail || String(i.mobile).trim() === inputMobile) &&
            (i.status === 'active' || i.status === 'pending_verification') &&
    ...
    login(email, mobile) {
        ...
        // Admin Check (Case-insensitive email, flexible password)
        // Primary: admin@thiranex.in | Secondary: thiranex.internships@outlook.com
        // Password: Thiranex@010525 (Case-insensitive for 'Thiranex' part)
        const isAdminEmail = cleanEmail.toLowerCase() === 'admin@thiranex.in' || 
                           cleanEmail.toLowerCase() === 'thiranex.internships@outlook.com';
        
        const isCorrectPassword = cleanMobile.substring(0, 8).toLowerCase() === 'thiranex' && 
                                 cleanMobile.substring(8) === '@010525';
