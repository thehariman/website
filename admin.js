...
function renderPayments() {
    ...
    // Show only pending payments; rejected entries are removed from the list
    let interns = DB.get('interns').filter(i => i.paymentStatus === 'pending' && i.status !== 'dropped');
...
async function verifyPayment(id, status) {
    ...
    if (await DB.verifyPayment(id, status)) {
        // If payment was rejected, prompt intern via WhatsApp to correct the transaction number BEFORE UI refresh to avoid popup blocker
        if (status === 'rejected' && intern) {
            const waMsg = `Hi ${intern.name}! Your payment could not be verified. Please update the correct 12‑digit transaction number in the portal.`;
            console.log('Opening WhatsApp for intern', intern.name, intern.mobile, waMsg);
            openWhatsApp(intern.mobile, waMsg);
        }
        refreshDashboard();
